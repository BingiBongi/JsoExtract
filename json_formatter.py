...
def generate_sql_extract_example(json_data):
    ...
    def generate_sql_for_dict(path, data):
        if isinstance(data, dict):
            ...
            sql_queries.append(f"SELECT JSON_EXTRACT(json_column, '$.{path}') FROM table_name;")
...
with col1:
    ...
    input_json = st.text_area("", height=350, max_chars=5000, key="input_json")
...
with col2:
    ...
    st.code(formatted_json, language="json", height=350)
